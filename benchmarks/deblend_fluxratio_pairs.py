#!/usr/bin/env python3
"""Standalone deblending test for high-contrast star pairs."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate SEP deblending on high-contrast close pairs."
    )
    ...
    parser.add_argument(
        "--flux-min", type=float, default=2000.0, help="Min bright flux."
    )
    parser.add_argument(
        "--flux-max", type=float, default=12000.0, help="Max bright flux."
    )
    parser.add_argument(
        "--flux-ratios",
        type=str,
        default="0.3,0.1,0.03,0.01",
        help="Comma-separated faint/bright flux ratios to test.",
    )
    parser.add_argument(
    ...
    parser.add_argument("--minarea", type=int, default=5, help="Minimum area.")
    parser.add_argument(
...
def classify_pair(
    bright: tuple[float, float],
    faint: tuple[float, float],
    det_x: np.ndarray,
    det_y: np.ndarray,
    match_radius: float,
) -> str:
    if det_x.size == 0:
        return "missed"
    d_b = np.hypot(det_x - bright[0], det_y - bright[1])
    d_f = np.hypot(det_x - faint[0], det_y - faint[1])
    set_b = set(np.where(d_b <= match_radius)[0])
    set_f = set(np.where(d_f <= match_radius)[0])
    union = set_b | set_f
    if not union:
        return "missed"
    if set_b and set_f:
        return "both" if len(union) >= 2 else "merged"
    if set_b:
        return "bright_over_split" if len(set_b) > 1 else "bright_only"
    if set_f:
        return "faint_over_split" if len(set_f) > 1 else "faint_only"
    return "missed"


def run_case(args: argparse.Namespace, flux_ratio: float) -> dict[float, dict[str, int]]:
    ...
    for sep_f in sep_fwhm:
        ...
        for _ in range(args.pairs_per_sep):
            cx, cy = choose_center(rng, centers, args.size, margin, min_center_sep)
            angle = rng.uniform(0.0, 2.0 * math.pi)
            dx = 0.5 * sep_pix * math.cos(angle)
            dy = 0.5 * sep_pix * math.sin(angle)
            bright = (cx + dx, cy + dy)
            faint = (cx - dx, cy - dy)
            f_bright = rng.uniform(args.flux_min, args.flux_max)
            f_faint = f_bright * flux_ratio
            add_gaussian_stamp(data, bright[0], bright[1], f_bright, sigma)
            add_gaussian_stamp(data, faint[0], faint[1], f_faint, sigma)
            pairs.append((sep_f, bright, faint))
    ...
    for sep_f, bright, faint in pairs:
        outcome = classify_pair(bright, faint, det_x, det_y, match_radius)
        counts[sep_f][outcome] += 1

    return counts


def print_report(args: argparse.Namespace, ratio: float, counts: dict[float, dict[str, int]]) -> None:
    print("Deblending high-contrast pair test")
    print(
        f"- ratio={ratio:.4f}, size={args.size}, fwhm={args.fwhm:.2f}, "
    ...
    print(
        ...
        f"deblend_fwhm={args.deblend_fwhm:.2f}, "
        f"deblend_method={args.deblend_method}"
    )
    print(
    ...
    for sep_f in sorted(counts.keys()):
        ...
        stats = {k: counts[sep_f].get(k, 0) / n for k in counts[sep_f].keys()}
        both = stats.get("both", 0.0)
        merged = stats.get("merged", 0.0)
        bright_only = stats.get("bright_only", 0.0)
        faint_only = stats.get("faint_only", 0.0)
        bright_over = stats.get("bright_over_split", 0.0)
        faint_over = stats.get("faint_over_split", 0.0)
        missed = stats.get("missed", 0.0)
        print(
            f"{sep_f:.2f} FWHM: both={both:.3f} merged={merged:.3f} "
            f"bright_only={bright_only:.3f} faint_only={faint_only:.3f} "
            f"bright_over={bright_over:.3f} faint_over={faint_over:.3f} "
            f"missed={missed:.3f}"
        )
        for key, value in counts[sep_f].items():
            totals[key] += value

    print("")
    total_pairs = sum(sum(v.values()) for v in counts.values())
    print("Overall (fractions)")
    print(
        f"both={totals['both'] / total_pairs:.3f} "
        f"merged={totals['merged'] / total_pairs:.3f} "
        f"bright_only={totals['bright_only'] / total_pairs:.3f} "
        f"faint_only={totals['faint_only'] / total_pairs:.3f} "
        f"bright_over={totals['bright_over_split'] / total_pairs:.3f} "
        f"faint_over={totals['faint_over_split'] / total_pairs:.3f} "
...
def main() -> int:
    args = parse_args()
    ratios = [float(x) for x in args.flux_ratios.split(",") if x.strip()]

    for ratio in ratios:
        counts = run_case(args, ratio)
        print_report(args, ratio, counts)
        if ratio != ratios[-1]:
            print("\n" + "-" * 72 + "\n")
